...
Phase 250 split S3 guided workflow detection into `guided-workflow-detection.ts`; Phase 294 split S4 canonical reply delivery into `guided-workflow-delivery.ts` (`sendCanonicalWorkflowResponse`, `presentGuidedWorkflowMoment`, `loadGuidedWorkflowConfig`, `shouldAutoPresent`, `GuidedWorkflowAutoPresent`, `GuidedWorkflowConfig`, and `GUIDED_WORKFLOW_DEFAULTS`). The split preserves option parsing, merge-gate routing detection, canonical reply evidence, and the no-inferred-merge-intent boundary without changing runtime semantics.
Phase 295 measured the S4 offload and ranked future TypeScript context-offload candidates in `docs/PI-NATIVE-CONTEXT-OFFLOAD-MEASUREMENT-NEXT-TARGET-RANKING.md`. That report is recommendation-only and does not authorize another runtime extraction.
Phase 254 split S7 PALS context injection into `pals-context-injection.ts`; S4 canonical reply delivery now lives in `guided-workflow-delivery.ts`. The split preserves the literal byte sequences `[PALS_STATE_AUTHORITY=.paul/STATE.md]` and `[PALS_ACTIVATION_SIGNAL]`, the legacy header `## PALS Context (auto-injected)`, the custom-message-type identity `pals-context`, and the `before_agent_start`/`context` Pi event surface without changing runtime semantics.
Phase 258 split S6 lifecycle UI into `lifecycle-ui.ts`; S4 canonical reply delivery now lives in `guided-workflow-delivery.ts`. The split preserves the runtime UI-element identifier byte sequence `pals-lifecycle` (the `PALS_STATUS_ID` / `PALS_WIDGET_ID` value), the loop-badge `PLAN✓ APPLY✓ UNIFY✓` shape from `[✓○]` marks, the action-label heuristics (`Complete` / `Blocked` / `Waiting` / `Ready` / `Next`), the module-activity `Modules: <stage> • <list>` render shape, and every existing `syncLifecycleUi(ctx)` call site (5 invocations) without changing runtime semantics. The no-UI-only-lifecycle-decisions invariant holds: `lifecycle-ui.ts` performs no `.paul/*` writes, no `pi.sendUserMessage` calls, and no S4 canonical-reply identifier invocations.
Phase 262 split S8 command routing into `command-routing.ts`; S4 canonical reply delivery now lives in `guided-workflow-delivery.ts`. The split preserves the twelve `/paul-*` command-registration call shapes (`paul-init`, `paul-plan`, `paul-apply`, `paul-unify`, `paul-resume`, `paul-status`, `paul-fix`, `paul-pause`, `paul-milestone`, `paul-discuss`, `paul-help`, `paul-review` — each with exact `name` / `description` / `skill` / `guidance` byte sequences in the `COMMANDS` array), the canonical `/paul-*` → `/skill:paul-*` translation rule in `routeCommand`, the four command-routing constants exact-string (`COMMAND_ACTIVATION_TURN_BUDGET = 3`, `PROMPT_ACTIVATION_TURN_BUDGET = 1`, `PRIMARY_QUICK_ACTION_LIMIT = 3`, `MAX_QUICK_ACTIONS = 5`), the `detectCommandSignal` regex literal byte sequence, the `getQuickActions` priority order, the five `Key.ctrlAlt("[nsrhm]")` shortcut invocations, and the registration loop call shape (12 iterations) byte-for-byte at the same call site in `pals-hooks.ts` without changing runtime semantics. The no-UI-only-lifecycle-decisions invariant holds: `command-routing.ts` performs no `.paul/*` writes, no `pi.sendUserMessage` calls outside the `routeCommand` factory body, and no S4 canonical-reply identifier invocations.
