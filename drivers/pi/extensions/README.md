...
- `~/.pi/agent/extensions/{name}.ts` for every repo-owned `drivers/pi/extensions/*.ts` sibling — the Pi-native `/paul-*` command layer, lifecycle hooks, guided workflow UX, and extracted helper siblings including `pals-hooks.ts`, `module-activity-parsing.ts`, `artifact-slice-rendering.ts`, `workflow-resource-capsule-rendering.ts`, `guided-workflow-detection.ts`, `pals-context-injection.ts`, `lifecycle-ui.ts`, `command-routing.ts`, and `plannotator-bridge.ts`
...
Phase 250 split S3 guided workflow detection into `guided-workflow-detection.ts`; S4 canonical reply delivery (`sendCanonicalWorkflowResponse`, `presentGuidedWorkflowMoment`, `loadGuidedWorkflowConfig`, and `shouldAutoPresent`) remains in `pals-hooks.ts`. The split preserves option parsing, merge-gate routing detection, canonical reply evidence, and the no-inferred-merge-intent boundary without changing runtime semantics.
Phase 254 split S7 PALS context injection into `pals-context-injection.ts`; S4 canonical reply delivery (`sendCanonicalWorkflowResponse`, `presentGuidedWorkflowMoment`, `loadGuidedWorkflowConfig`, and `shouldAutoPresent`) remains in `pals-hooks.ts`. The split preserves the literal byte sequences `[PALS_STATE_AUTHORITY=.paul/STATE.md]` and `[PALS_ACTIVATION_SIGNAL]`, the legacy header `## PALS Context (auto-injected)`, the custom-message-type identity `pals-context`, and the `before_agent_start`/`context` Pi event surface without changing runtime semantics.
Phase 258 split S6 lifecycle UI into `lifecycle-ui.ts`; S4 canonical reply delivery (`sendCanonicalWorkflowResponse`, `presentGuidedWorkflowMoment`, `loadGuidedWorkflowConfig`, `shouldAutoPresent`) remains in `pals-hooks.ts`. The split preserves the runtime UI-element identifier byte sequence `pals-lifecycle` (the `PALS_STATUS_ID` / `PALS_WIDGET_ID` value), the loop-badge `PLAN✓ APPLY✓ UNIFY✓` shape from `[✓○]` marks, the action-label heuristics (`Complete` / `Blocked` / `Waiting` / `Ready` / `Next`), the module-activity `Modules: <stage> • <list>` render shape, and every existing `syncLifecycleUi(ctx)` call site (5 invocations) without changing runtime semantics. The no-UI-only-lifecycle-decisions invariant holds: `lifecycle-ui.ts` performs no `.paul/*` writes, no `pi.sendUserMessage` calls, and no S4 canonical-reply identifier invocations.
Phase 262 split S8 command routing into `command-routing.ts`; S4 canonical reply delivery (`sendCanonicalWorkflowResponse`, `presentGuidedWorkflowMoment`, `loadGuidedWorkflowConfig`, `shouldAutoPresent`) remains in `pals-hooks.ts`. The split preserves the twelve `/paul-*` command-registration call shapes (`paul-init`, `paul-plan`, `paul-apply`, `paul-unify`, `paul-resume`, `paul-status`, `paul-fix`, `paul-pause`, `paul-milestone`, `paul-discuss`, `paul-help`, `paul-review` — each with exact `name` / `description` / `skill` / `guidance` byte sequences in the `COMMANDS` array), the canonical `/paul-*` → `/skill:paul-*` translation rule in `routeCommand`, the four command-routing constants exact-string (`COMMAND_ACTIVATION_TURN_BUDGET = 3`, `PROMPT_ACTIVATION_TURN_BUDGET = 1`, `PRIMARY_QUICK_ACTION_LIMIT = 3`, `MAX_QUICK_ACTIONS = 5`), the `detectCommandSignal` regex literal byte sequence, the `getQuickActions` priority order, the five `Key.ctrlAlt("[nsrhm]")` shortcut invocations, and the registration loop call shape (12 iterations) byte-for-byte at the same call site in `pals-hooks.ts` without changing runtime semantics. The no-UI-only-lifecycle-decisions invariant holds: `command-routing.ts` performs no `.paul/*` writes, no `pi.sendUserMessage` calls outside the `routeCommand` factory body, and no S4 canonical-reply identifier invocations.
Phase 285 adds `plannotator-bridge.ts` as an optional/advisory bridge helper for the Phase 284 Plannotator contract surface. It exposes `requestPlanReview`, `requestCodeReview`, and `awaitReviewResult` over `plannotator:request` / `plannotator:review-result`, treats `savedPath` as advisory only, ignores `agentSwitch` and `permissionMode`, writes no `.paul/*` lifecycle truth (no .paul/* lifecycle truth), and does not change workflow instructions yet. Plannotator review output remains evidence for PAUL-owned workflows, not approval, routing, permission, or lifecycle authority.
...
| Pi extension(s) | `~/.pi/agent/extensions/{name}.ts` for every `*.ts` file under repo `drivers/pi/extensions/` | Command registration, lifecycle hooks, guided workflow UX; the install surface is a source-set rule over repo-owned `drivers/pi/extensions/*.ts` files (currently `pals-hooks.ts`, the Phase 239 first-spike S5 extraction `module-activity-parsing.ts`, the Phase 243 S1 sibling extraction `artifact-slice-rendering.ts`, the Phase 246 S2 sibling extension `workflow-resource-capsule-rendering.ts`, the Phase 250 S3 detection sibling `guided-workflow-detection.ts`, the Phase 254 S7 PALS context injection sibling `pals-context-injection.ts`, the Phase 258 S6 lifecycle UI sibling `lifecycle-ui.ts`, the Phase 262 S8 command routing sibling `command-routing.ts`, and the Phase 285 Plannotator bridge sibling `plannotator-bridge.ts` per `docs/PI-NATIVE-EXTENSION-MODULARIZATION-CONTRACT.md` and `docs/PALS-PLANNOTATOR-UI-INTEGRATION-CONTRACT.md`); future extracted siblings install without per-file installer edits |
...
The Pi installer treats `drivers/pi/extensions/*.ts` as a **source set**: every repo-owned `*.ts` sibling under `drivers/pi/extensions/` is installed by basename into `~/.pi/agent/extensions/`, and the uninstaller removes only those repo-owned basenames (it never broad-deletes unrelated Pi extensions under `~/.pi/agent/extensions/`). This generalizes the earlier per-submodule special case for `pals-hooks.ts` plus `module-activity-parsing.ts` (now joined by the Phase 243 S1 sibling extraction `artifact-slice-rendering.ts`, the Phase 246 S2 sibling extension `workflow-resource-capsule-rendering.ts`, the Phase 250 S3 detection sibling `guided-workflow-detection.ts`, the Phase 254 S7 PALS context injection sibling `pals-context-injection.ts`, the Phase 258 S6 lifecycle UI sibling `lifecycle-ui.ts`, the Phase 262 S8 command routing sibling `command-routing.ts`, and the Phase 285 Plannotator bridge sibling `plannotator-bridge.ts`), so future extracted extension siblings install without further `install.sh` / `uninstall.sh` edits.
