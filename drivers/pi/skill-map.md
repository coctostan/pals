...
- guided workflow UX is an adapter-side reply-routing aid, not a standalone skill or workflow authority: S3 detection lives in `guided-workflow-detection.ts`; S4 canonical reply delivery lives in `guided-workflow-delivery.ts`; S7 PALS context injection lives in `pals-context-injection.ts`; S6 lifecycle UI lives in `lifecycle-ui.ts`; S8 command routing lives in `command-routing.ts`; `pals-hooks.ts` remains the runtime registration/orchestration entry point. Pi may surface `plan-review`, `apply-approval`, checkpoint, resume-next, continue-to-UNIFY, phase/milestone transition, and `merge-gate-routing` prompts, but it sends canonical transcript replies only after explicit confirm/select user action; notify-only mode sends no reply, and no auto-approval, no auto-continue, no skipped checkpoints, no UI-only lifecycle decisions, and no inferred merge intent are allowed
- Phase 295 report `docs/PI-NATIVE-CONTEXT-OFFLOAD-MEASUREMENT-NEXT-TARGET-RANKING.md` measures the S4 offload and ranks future TypeScript context-offload candidates; it is recommendation-only and does not authorize another runtime extraction.
