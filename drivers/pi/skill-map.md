...
- `~/.pi/agent/extensions/{name}.ts` for repo-owned `drivers/pi/extensions/*.ts` siblings (including the compatibility anchor `~/.pi/agent/extensions/pals-hooks.ts`) provides the user-facing `/paul-*` command layer, lifecycle hooks, bounded shortcut UX, and extracted helpers (`pals-hooks.ts`, `module-activity-parsing.ts`, `artifact-slice-rendering.ts`, `workflow-resource-capsule-rendering.ts`, `guided-workflow-detection.ts`, `pals-context-injection.ts`, `lifecycle-ui.ts`, `command-routing.ts`, `plannotator-bridge.ts`)
...
- guided workflow UX is an adapter-side reply-routing aid, not a standalone skill or workflow authority: S3 detection lives in `guided-workflow-detection.ts`, while S4 canonical reply delivery remains in `pals-hooks.ts`; S7 PALS context injection lives in `pals-context-injection.ts`, while S4 canonical reply delivery remains in `pals-hooks.ts`; S6 lifecycle UI lives in `lifecycle-ui.ts`, while S4 canonical reply delivery remains in `pals-hooks.ts`; S8 command routing lives in `command-routing.ts`, while S4 canonical reply delivery remains in `pals-hooks.ts`; Pi may surface `plan-review`, `apply-approval`, checkpoint, resume-next, continue-to-UNIFY, phase/milestone transition, and `merge-gate-routing` prompts, but it sends canonical transcript replies only after explicit confirm/select user action; notify-only mode sends no reply, and no auto-approval, no auto-continue, no skipped checkpoints, no UI-only lifecycle decisions, and no inferred merge intent are allowed
- the Plannotator bridge is an optional/advisory adapter helper in `plannotator-bridge.ts`, not a standalone skill, module, or workflow authority: it exposes `requestPlanReview`, `requestCodeReview`, and `awaitReviewResult` for the Phase 284 contract event surface, treats `savedPath` as advisory only, ignores `agentSwitch` and `permissionMode`, writes no `.paul/*` lifecycle truth (no .paul/* lifecycle truth), and cannot approve, route, or change PAUL workflow state
...
- `~/.pi/agent/extensions/{name}.ts` for every repo-owned `drivers/pi/extensions/*.ts` sibling — Pi extension surface (command registration, lifecycle hooks, guided workflow UX); the compatibility anchor remains `~/.pi/agent/extensions/pals-hooks.ts`, and the installer applies a source-set rule over `drivers/pi/extensions/*.ts`, currently `pals-hooks.ts`, the Phase 239 first-spike S5 extraction `module-activity-parsing.ts`, the Phase 243 S1 sibling extraction `artifact-slice-rendering.ts`, the Phase 246 S2 sibling extraction `workflow-resource-capsule-rendering.ts`, the Phase 250 S3 detection sibling `guided-workflow-detection.ts`, the Phase 254 S7 PALS context injection sibling `pals-context-injection.ts`, the Phase 258 S6 lifecycle UI sibling `lifecycle-ui.ts`, the Phase 262 S8 command routing sibling `command-routing.ts`, and the Phase 285 Plannotator bridge sibling `plannotator-bridge.ts` per `docs/PI-NATIVE-EXTENSION-MODULARIZATION-CONTRACT.md` and `docs/PALS-PLANNOTATOR-UI-INTEGRATION-CONTRACT.md`. The uninstaller mirrors the source set and never broad-deletes unrelated Pi extensions. Driver manifest audit (Phase 241): `drivers/pi/driver.yaml` already lists `extensions/` as a directory entry under `files`, which is sufficient for multi-file extension packaging — no per-file manifest edit is required, and `driver.yaml` remains unchanged unless contrary evidence arises later.
