...
Pi is the supported runtime: PALS installs canonical skills under `~/.pi/agent/skills/pals/`, Pi extension siblings under `~/.pi/agent/extensions/` (currently `pals-hooks.ts` plus the Phase 239 S5 extraction `module-activity-parsing.ts`, the Phase 243 S1 sibling extraction `artifact-slice-rendering.ts`, the Phase 246 S2 sibling extraction `workflow-resource-capsule-rendering.ts`, and the Phase 250 S3 detection sibling `guided-workflow-detection.ts`, per the source-set rule in `docs/PI-NATIVE-EXTENSION-MODULARIZATION-CONTRACT.md`), and project-shipped helper agents under `~/.pi/agent/agents/`.
...
Validation Classification makes validation purpose explicit before any cleanup: current suite families map to Pi-Supported Runtime, Shared Invariant, Frozen Legacy Parity, Runtime Lens, Guided UI Safety, Helper Delegation, GitHub Flow Safety, and Artifact / Process. The current post-v2.51 / pre-Phase-224 command baseline is Pi 203/203 and cross-harness 119/119; reports, docs, widgets, summaries, helper reports, and Pi surfaces may explain that posture but fresh validation command output remains the only pass/fail authority.
...
- **Default supported target:** Pi skill/kernel surface, extension source-set surface, and project-agent surface (`~/.pi/agent/skills/pals/`, `~/.pi/agent/extensions/{name}.ts` for repo `drivers/pi/extensions/*.ts` including `pals-hooks.ts` and `guided-workflow-detection.ts`, `~/.pi/agent/agents/`)
