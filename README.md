...
- **HTML Presentation Packets** — Optional, static, self-contained HTML human-review briefs for milestone, PLAN, APPLY, and UNIFY moments; default-skip, non-blocking, and main-session only, derived from cited authoritative artifacts and never deciding lifecycle state (`.paul/*` and PLAN/APPLY/UNIFY remain authoritative)
