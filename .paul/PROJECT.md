...
- [~] v2.41 CODI v0.1 — Natural-Scope Evidence & Signal-1/2 Validation — in progress. Proposal approved at `.paul/proposals/v2.41-cross-repo-trial-quark.md`. Phase 178 APPLY executed via `.paul/phases/178-codi-cross-repo-install/178-01-PLAN.md`: `quark/pals.json` now enables `modules.codi`, `quark/.codegraph/graph.db` was refreshed, `quark/.paul/CODI-HISTORY.md` was seeded with the canonical zero-row header, and a direct smoke test resolved `deriveTools` to `src/kernel/context.ts:40` (`assembleContext`) with behavioral depth 1. UNIFY is still pending before Phase 179 opens the natural observation window. Phase 179 trial fuel remains the user's in-flight quark patches, authored before CODI was enabled in quark — causally independent, zero selection bias. Phase 180 re-trial applies the unchanged Phase 173/177 rubric to resolve Q2.
## Constraints
...
---
*Created: 2026-03-11*
*Last updated: 2026-04-18 after Phase 178 APPLY (v2.41 CODI cross-repo install executed in quark; UNIFY pending before Phase 179 observation opens).*
