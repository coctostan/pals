...
| 2026-06-05/06: v2.72 Phases 303–304 fixed then executed the shared-runtime-helper extraction. | Phases 303–304 (PRs #223 `ee5a9010`, #224 `11c8112f`) | `docs/PALS-SHARED-RUNTIME-HELPERS-CONTRACT.md` authoritative for v2.72. Phase 304 moved 7 pure/derived helpers + `PalsStateSnapshot` from `pals-hooks.ts` into a Node-only leaf `shared-runtime-helpers.ts`, repointed all 9 importers, removed the inverted helper-hub edge — acyclic one-way graph, zero behavior change. Validation closure = Phase 305. |
...
*Created: 2026-03-11*
*Last updated: 2026-06-06 after v2.72 Phase 304 (shared-runtime-helpers extraction + import repoint, PR #224); Phase 305 (validation + closure) pending.*
