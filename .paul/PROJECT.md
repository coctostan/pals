...
No active milestone. Latest completed: **v2.72 Shared Runtime Helper Extraction** — all 3 phases (303–305) complete, closed 2026-06-07 (PR #225 `ad9ea9dd`, tag `v2.72`). Extracted 7 pure/derived filesystem + state helpers + `PalsStateSnapshot` out of `pals-hooks.ts` into a Node-only dependency leaf `shared-runtime-helpers.ts`, repointed all 9 importers, and removed the inverted helper-hub edge for an acyclic one-way Pi extension graph (zero behavior change); recorded the contract closure and the 8th cross-harness closure-guardrail. Run `/paul:discuss-milestone` or `/paul:milestone` for the next milestone (standing candidate: validation-suite decomposition).
...
- v2.43/v2.44 context optimization preserves archive links, historical authority, module evidence, GitHub Flow safety, and parent-owned APPLY while reducing recurring read cost with anti-regrowth validation.
- v2.45 established Pi-native runtime assistance as the primary architecture direction: artifact-slice loading, workflow/resource capsules, guided workflow evidence, and delegated APPLY packets, while keeping `.paul/*` authoritative.
...
- v2.62 compacted the four hot lifecycle artifacts to per-section byte budgets and added Selective Artifact Loading + anti-regrowth TAPs, without weakening lifecycle authority, helper compatibility, validation truth, or full read fallback.
...
| v2.69 Pi Runtime Context Offload — S4 First | Complete (3 of 3 phases) — closed 2026-05-22 via PR #214 | Phase 293 S4 offload contract; Phase 294 extracted `guided-workflow-delivery.ts`; Phase 295 measured offload (`pals-hooks.ts` 671→546) and ranked shared helper extraction next. |
...
| v2.72 Shared Runtime Helper Extraction | Complete (3 of 3 phases) — closed 2026-06-07 via PR #225 `ad9ea9dd` (tag `v2.72`) | Extracted 7 pure/derived filesystem + state helpers + `PalsStateSnapshot` out of `pals-hooks.ts` into a Node-only dependency leaf `shared-runtime-helpers.ts`, repointed all 9 importers, removed the inverted helper-hub edge (acyclic one-way graph, zero behavior change); `## Status After Phase 305` contract closure + 8th XH closure-guardrail (253→254). PRs #223/#224/#225. Final: Pi e2e 343/343, XH 254/254. Detail: [v2.72 archive](archive/roadmap/v2.72-shared-runtime-helper-extraction.md). |
...
Current milestone pointer: **None active.** Latest closed: **v2.72 Shared Runtime Helper Extraction** (Phases 303–305; PRs #223/#224/#225; tag `v2.72`). Run `/paul:discuss-milestone` or `/paul:milestone`; standing candidate is validation-suite decomposition.
...
| 2026-05-08: Single-Write-per-artifact recipe ratified for context-diet contracts and large lifecycle-artifact compaction. | Phase 265 / Phase 266 | One `Write` per file overwrite (computed offline) replaces many sequential `Edit`s for large markdown artifacts; Edit gets brittle on hash-drift. |
| 2026-05-08: Hot/warm/cold split rules and per-section byte budgets are the v2.62 authoritative contract. | Phase 265 (`docs/PALS-STATE-CONTEXT-DIET-CONTRACT.md`) | STATE ≤ 20 KB / PROJECT ≤ 25 KB / MILESTONES ≤ 18 KB / ROADMAP ≤ 12 KB AND ≤ 120 lines / aggregate ≤ 80 KB. Just-closed + active rows stay hot; older lifts to archive. Phase 267 ratifies anti-regrowth TAPs. |
| 2026-05-08: Cross-file narrative duplication recorded as deferred debt. | Phase 265 | Remediation needs an architectural single-source choice; deferred to v2.63+; v2.62 stays bounded to archive-and-compact. |
| 2026-05-08: Phase 267 implemented Selective Artifact Loading and closed v2.62 while preserving full authoritative read fallback. | Phase 267 Plan 267-01 | Workflows locate relevant headings/markers/rows first and read bounded windows by default; full read remains an explicit fallback for rewrite/audit/repair/migration/lifecycle-write/whole-artifact validation. Anti-regrowth TAPs protect budgets and markers. |
| 2026-05-09: Phase 268 made resume context-efficiency operational rather than marker-only. | Phase 268 Plan 268-01 | Resume derives lifecycle position from bounded STATE windows first, treats handoffs/resume files as conditional fallback, removes broad/eager routine reads, with semantic guardrails rejecting wording-only compliance. |
| 2026-05-09: Phase 271 made UNIFY reconciliation bounded, SUMMARY-first, and semantically guarded. | Phase 271 Plan 271-01 | UNIFY loads bounded STATE/session/PLAN/execution evidence, keeps SUMMARY as durable reconciliation, records compact mandatory post-unify module reports, preserves ordered GitHub Flow gates, and requires last-plan transition routing. |
| 2026-05-10: Phase 272 closed v2.63 with bounded PAUSE and compact agent handoffs. | Phase 272 Plan 272-01 | `pause-work.md` starts from bounded STATE windows, writes compact agent handoff packets, archives stale handoffs, records `wip_result`, and validates PAUSE semantics in Pi/cross-harness suites. |
...
- Roadmap closure (v2.72): [.paul/archive/roadmap/v2.72-shared-runtime-helper-extraction.md](archive/roadmap/v2.72-shared-runtime-helper-extraction.md)
...
*Created: 2026-03-11*
*Last updated: 2026-06-07 after v2.72 Shared Runtime Helper Extraction complete (Phases 303–305; closed via PR #225 `ad9ea9dd`, tag `v2.72`). No active milestone.*
