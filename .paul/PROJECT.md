...
v2.56 Pi Extension Submodule Extraction Wave is **active**. Phase 242 Extension Extraction Target Baseline is **complete** (closed 2026-05-01 via PR #157): `docs/PI-NATIVE-EXTENSION-EXTRACTION-TARGET-BASELINE.md` is the source-cited Phase 243 handoff. Approved Phase 243 wave: S1 `artifact-slice-rendering` and S2 `workflow-resource-capsule-rendering`, with the explicit rule that Phase 243 may extract one or more approved candidates only when contracts/imports/install behavior/validation/reviewability remain bounded. S5 is already extracted (Phase 239); S3 is conditional secondary; S4/S6/S7/S8 are deferred by default. Validation stable: Pi `1..212/0`, cross-harness `1..127/0`, artifact_consistency PASS. Phase 243 Bounded Submodule Extraction Wave is the next planning target.
...
| v2.56 Pi Extension Submodule Extraction Wave | In Progress (1 of 4 phases) | Continue extracting bounded Pi extension submodules from `pals-hooks.ts` using the v2.55 generalized install surface. Phase 242 baseline complete; Phase 243 Bounded Submodule Extraction Wave, Phase 244 Documentation + Validation Surfacing, and Phase 245 Evidence Closure remain. |

Current milestone pointer: v2.56 Pi Extension Submodule Extraction Wave is active. Phase 242 closed 2026-05-01 via PR #157; Phase 243 Bounded Submodule Extraction Wave is ready to plan against `docs/PI-NATIVE-EXTENSION-EXTRACTION-TARGET-BASELINE.md` (S1 + S2 default approved wave; one-or-more multi-module rule when bounded). Phase 244 Documentation + Validation Surfacing and Phase 245 Evidence Closure follow.
...
*Created: 2026-03-11*
*Last updated: 2026-05-01 after Phase 242 Extension Extraction Target Baseline UNIFY (Phase 242 / PR #157); v2.56 Pi Extension Submodule Extraction Wave 1 of 4 phases complete.*