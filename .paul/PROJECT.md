...
Active milestone: **v2.66 Plannotator Integration — Rich UI Review Surfaces**. Phases 284–286 are complete: Phase 284 established the authoritative PALS/Plannotator contract, Phase 285 added the optional/advisory Pi bridge helper surface, and Phase 286 integrated opt-in plan review workflow semantics with advisory sidecars. Phase 287 remains for code-review workflow integration.
...
| v2.66 Plannotator Integration — Rich UI Review Surfaces | Active (3 of 4 phases complete) — Phase 284 closed 2026-05-14 via PR #202; Phase 285 closed 2026-05-14 via PR #203; Phase 286 closed 2026-05-14 via PR #204 | Integrates PALS with `@plannotator/pi-extension` for opt-in rich UI review while preserving `.paul/*` lifecycle authority. Phase 284 created the authoritative contract for consumed event/result fields, advisory sidecars, non-adoption, opt-in, failure modes, and validation classification. Phase 285 added the optional/advisory `drivers/pi/extensions/plannotator-bridge.ts` helper surface plus docs/skill-map surfacing and Pi marker validation. Phase 286 added opt-in `/paul:plan` review semantics, advisory plan-review sidecars, APPLY advisory-note loading, and Pi/cross-harness workflow marker validation. |
...
| 2026-05-14: Plan-review workflow integration remains PAUL-owned and advisory-only. | Phase 286 Plan 286-01 (`286-01-SUMMARY.md`; PR #204) | `/paul:plan` prompts per invocation (`Run Plannotator review? [y/n]`), decline is zero-overhead, approval/denial feedback is written only to advisory sidecars, `savedPath` is never adopted as PLAN.md, and APPLY reads `PLAN-REVIEW-NOTES.md` only as non-overriding context. |
...
*Last updated: 2026-05-14 after Phase 286 completion; v2.66 active.*
