...
| 2026-06-05: v2.72 Phase 303 fixed the shared-runtime-helper extraction boundary contract-first before any code moves. | Phase 303 Plan 303-01 (`303-01-SUMMARY.md`; PR #223 squash `ee5a9010`) | `docs/PALS-SHARED-RUNTIME-HELPERS-CONTRACT.md` is authoritative for v2.72: 7 pure/derived helpers + `PalsStateSnapshot` type move from `pals-hooks.ts` into a dependency-leaf `drivers/pi/extensions/shared-runtime-helpers.ts` (Node `fs`/`path` only, never imports siblings); source-verified inventory (8 exports) + importer map (12 sites across 8 siblings) with stay-put symbols (`collectRecentAssistantTexts`, `MAX_*`, `ActivationState`, ctx/event + CARL/activation/routing) explicitly excluded. Docs-only; extraction = Phase 304, validation = Phase 305. |
