...
if [ ! -d "$PALS_ROOT/kernel/workflows" ] || [ ! -d "$PALS_ROOT/modules" ]; then
  echo "ERROR: install.sh must be run from the PALS repo root."
  echo "Expected directories: kernel/workflows/, modules/"
  exit 1
fi
...
# ── 2. Install kernel commands ───────────────────────────────────
mkdir -p ~/.claude/commands/paul
...
# ── 3. Install modules (manifest-driven) ─────────────────────────
