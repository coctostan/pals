...
## Status After Phase 254

This contract closed for Phase 254. The S7 extraction has been executed against this boundary with no boundary relaxation. PR #169 squash-merged 2026-05-04 as `af1c1793`.

- **Six S7 functions extracted** (`buildPalsContextPayload`, `shouldInjectPalsContext`, `isPalsContextMessage`, `isLegacyPalsContextMessage`, `keepOnlyLatestPalsContextMessage`, `messagesChanged`) into `drivers/pi/extensions/pals-context-injection.ts` with no signature change at any call site; the five S7 call sites in `pals-hooks.ts` were preserved functionally identical.
- **Six S7 markers single-defined** as exact-string constants (`PRIMARY_INJECTION_EVENT = "before_agent_start"`, `SUPPORTING_CONTEXT_EVENT = "context"`, `PALS_CONTEXT_CUSTOM_TYPE = "pals-context"`, `LEGACY_PALS_CONTEXT_HEADER = "## PALS Context (auto-injected)"`, `STATE_AUTHORITY_TAG = "[PALS_STATE_AUTHORITY=.paul/STATE.md]"`, `ACTIVATION_SIGNAL_TAG = "[PALS_ACTIVATION_SIGNAL]"`) inside the new sibling; `pals-hooks.ts` references only imports of these names, never inline definitions.
- **Loader-compat invariant satisfied:** `pals-context-injection.ts` exports a default no-op factory carrying the literal `No-op Pi extension factory` JSDoc phrase; the Pi loader continues to treat the file as a sibling extension without registering duplicate command/hook surfaces.
- **Helper-import tightening applied:** the sibling value-imports only the helpers it uses (`renderArtifactSlices`, `renderWorkflowResourceCapsules`) and type-imports `ActivationState` and `PalsStateSnapshot` from `./pals-hooks` (erased at runtime, dependency graph stays acyclic).
- **Transcript-visible authority/activation byte sequences preserved exactly,** including `[PALS_STATE_AUTHORITY=.paul/STATE.md]`, `[PALS_ACTIVATION_SIGNAL]`, the legacy-header `## PALS Context (auto-injected)`, and the custom-message-type identity `pals-context`. Legacy-header migration (replace-then-emit-current) preserved.
- **Validation evidence:** install reports `[ok] Pi extensions installed: 6 files` (5 → 6); Pi e2e `228/228` (227 → 228 with the bounded `EXT_PALS_CONTEXT_INJECTION` TAP block plus the in-flight repoint of pre-existing assertion 133 from the conjunct `'PRIMARY_INJECTION_EVENT = "before_agent_start"'` definition string to the `'PRIMARY_INJECTION_EVENT'` imported-identifier reference); cross-harness `135/135` unchanged; artifact_consistency PASS; `git diff --check` clean.
- **Phase 255 owns user-facing-doc surfacing** of this S7 split (root README, Pi extensions README, Pi skill-map, modularization contract docs, this contract, and three new Pi validation TAP guardrails); Phase 256 will own evidence closure and the next-roadmap decision that closes v2.59. S4 canonical reply delivery, S6 lifecycle UI, and S8 command routing remain explicitly deferred. Full Claude Code / Agent SDK driver removal remains blocked per `docs/PI-NATIVE-LEGACY-RETENTION-INSTALL-SURFACE-CLEANUP-CONTRACT.md`.
