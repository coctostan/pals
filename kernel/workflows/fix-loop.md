...
1. Read `modules.yaml` (installed module registry; see `kernel/references/module-dispatch.md`). If not found, emit `[dispatch] post-apply: modules.yaml NOT FOUND — WARNING` and skip dispatch.
...
1. Read `modules.yaml` (installed module registry; see `kernel/references/module-dispatch.md`), or confirm already loaded. If not found, emit `[dispatch] post-unify: modules.yaml NOT FOUND — WARNING` and skip dispatch.
