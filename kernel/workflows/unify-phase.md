...
5. **Push latest changes** (SUMMARY.md and STATE.md updates are on feature branch):
   ...
   git push origin feature/${PHASE_NAME} 2>/dev/null || true
   ```
...
6. **Gate checks (strict order — stop at first failure):**
   ...
   **Gate 1 — PR exists:**
   ```bash
   PR_JSON=$(gh pr view feature/${PHASE_NAME} --json url,state,reviewDecision 2>/dev/null || echo "")
   ```
   - If empty (no PR):
     ...
     - If not auto_pr:
       ```
       ⛔ MERGE GATE: No PR exists for feature/${PHASE_NAME}
   ...
   **Gate 4 — PR merged:**
   ...
   - If `$PR_STATE` != "MERGED":
     ...
     - If "1":
       ```bash
       gh pr merge feature/${PHASE_NAME} --${GIT_MERGE_METHOD} --delete-branch
       ```
   ...
   **Gate 6 — Feature branch cleanup (if delete_branch_on_merge=true):**
   ```bash
   git branch -d feature/${PHASE_NAME} 2>/dev/null || true
   ```
   Display: `Cleaned up feature/${PHASE_NAME}` or `Branch already removed`
